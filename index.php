    ...
    if (!empty($_POST['reply'])) {
        ...
        if (isset($_POST['post_text']) && !empty($_POST['post_text'])) {
            ...
            $sql = 'insert into bulletinboard
                   (send_date, post_text, post_image, send_user_id, reply_flag)
             values (:date, :post_text, null, :send_user_id, false)';
            $stmt = $dbh->prepare($sql);
            $stmt->bindValue(':date', $today);
            $stmt->bindValue(':post_text', $_POST['post_text']);
            $stmt->bindValue(':send_user_id', $user_id['user_id']);
            $stmt->execute();
        }
...
/*
    $stmtbuf = $dbh->prepare($sql);
    $stmtbuf->execute();

    while($res = $stmtbuf->fetch(PDO::FETCH_ASSOC)){
        if ($res['reply_flag']) {
            $sql2 = 'select * from replyboard where src_post_id = :src_post_id';            var_dump($res['post_id']);
            $stmt2 = $dbh->prepare($sql2);
            $stmt2->bindValue(':src_post_id', $res['post_id'], PDO::PARAM_INT);
            $stmt2->execute();
        }
    }
*/
...
<form action="" method="post" name="post_text">
